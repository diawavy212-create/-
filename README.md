...
- 思政培训：支持后台发布、编辑、删除培训；教师端查看时间、地点、名额并报名或取消报名；后台可查看报名名单。
...
生产环境请通过 `ADMIN_LOGIN_PASSWORD` 修改默认密码，并保持管理员账号已绑定 `teacher.cas_account` 或 `teacher.user_id`。

### 微信小程序

用微信开发者工具打开 `miniprogram` 目录。公开仓库中的 `appid` 使用 `touristappid` 占位，真实小程序 AppID 请放在本地私有配置中。小程序接口地址集中配置在 `miniprogram/config.js` 的 `DEFAULT_API_BASE_URL`。
