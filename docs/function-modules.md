...
| --- | --- | --- | --- |
| 教师端 | 查看本人基础信息，后续维护非敏感联系方式 | 提交诉求，查看进度和结果，评价满意度 | 查看培训时间、地点和名额，提交或取消报名，查看个人台账 |
...
| 微信注册 | `POST /api/v1/auth/wechat-register` |
| 后台账号密码登录 | `POST /api/v1/auth/admin-login` |
| 后台 CAS 登录 | `POST /api/v1/auth/cas-login` |
